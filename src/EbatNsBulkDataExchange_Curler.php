...
class EbatNsBulkDataExchange_Curler
{
    ...
    public function parse($responseParser = null)
    {
        ...
        if ($results && $results[0]->getContent() instanceof EbatNsBulkDataExchange_ResponseError) {
        } elseif (!$handles) {
        } elseif (!$mh) {
            $ch = $handles[0];
            $raw = curl_exec($ch);
            ...
            if ($raw !== false && $raw !== '') {
                $results[] = EbatNsBulkDataExchange_Decoder::decode($raw, $parser);
            } else {
                $message = $errorNumber ? "curl: $errorNumber " . curl_error($ch) : 'Empty response';
                $error = new EbatNsBulkDataExchange_ResponseError($message);
            }
        } else {
            do {
                ...
                if ($status !== CURLM_OK) {
                    $error = new EbatNsBulkDataExchange_ResponseError('curl(m): ' . curl_multi_strerror($status));
                    $handles = [];
                    break;
                }
            } while ($running);
        ...
        if ($error) {
            $results[] = new EbatNsBulkDataExchange_DecodedResponse($error);
        }
    ...
    private function parseFromCurlBlockedApi($parser)
    {
        ...
        foreach ($this->customHandles as $ch) {
            ...
            if ($fp === false) {
                $results[] = new EbatNsBulkDataExchange_ResponseError("Problem with socket-connect ($host)");
                break;
            }
            ...
            if ($transferEncoding) {
                $error = new EbatNsBulkDataExchange_ResponseError("Transfer encoding not supported: $transferEncoding, Size: $contentLength");
                break;
            }
            ...
            if ($raw === '') {
                $error = new EbatNsBulkDataExchange_ResponseError('Empty response from: ' . $ch->url . ' ' . $ch->headers[0]);
                break;
            }

            $results[] = EbatNsBulkDataExchange_Decoder::decode("$headerText\r\n$raw", $parser);
        }
