...
class EbatNsBulkDataExchange_ResponseParser
{
    protected $typeNs;
    ...
    public function parse($xml, $tag = null, $type = null, $errorParsingEnabled = true)
    {
        ...
        if ($xmlParserError) {
            return new EbatNsBulkDataExchange_ResponseError($xmlParserError, 90000 + 1);
        }
        ...
        if (!$errorParsingEnabled) {
            return new EbatNsBulkDataExchange_ResponseError($xml, 90000 + 11);
        }

        return EbatNsBulkDataExchange_ResponseError::parseError($xml, $this);
    }
    ...
    protected function _startElement($parser, $prefixedName, $attrs)
    {
        ...
        if (strtolower($prefixedName) === 'soapenv:fault') { // fault found before in response
            ...
            $isEnum = $nsTypeName && is_subclass_of($nsTypeName,'InTradeSys\eBay\bulkDataExchange\EbatNsBulkDataExchange_EnumType');
    ...
    {
        $base = strpos($name, '\\') ? $name : 'InTradeSys\eBay\bulkDataExchange\\' . ucfirst($name);
    ...
    {
        $response = new EbatNsBulkDataExchange_ResponseError();
        $code = '';
        $severity = '';
