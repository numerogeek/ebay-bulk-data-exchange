...
class EbatNsBulkDataExchange_Decoder
{
    public static function decode($raw, $parser = null)
    {
        ...
        if (count($match) !== 2) {
            return new EbatNsBulkDataExchange_DecodedResponse($raw);
        }
        ...
        $result = self::parseContent(new EbatNsBulkDataExchange_DecodedResponse($isMultipart ? null : $raw, $assocHeaders), $parser);
        ...
        while ($eof !== true) {
            ...
            if (count($match) === 2) {
                ...
                $file = new EbatNsBulkDataExchange_DecodedResponse($body, self::parseHeaders($header));
        ...
        if (!$result->getContent() && !$result->getFiles()) {
            $result->setContent(new EbatNsBulkDataExchange_ResponseError('Was not able to decode the response!'));
        }
