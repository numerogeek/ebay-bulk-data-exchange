...
abstract class EbatNsBulkDataExchange_AbstractClient
{
    ...
    */
    ...
    /**
    * @var EbatNsBulkDataExchange_DataConverter
    */
    protected $dataConverter = null;


    public function __construct($session, $dataConverter = 'EbatNsBulkDataExchange_DataConverterUtf8')
    {
        if (is_string($dataConverter)) {
            $dataConverter = (strpos($dataConverter, '\\') === false ? 'InTradeSys\eBay\bulkDataExchange\\' : '') . $dataConverter;
            $dataConverter = new $dataConverter();
        }

        if ($session->getUseStandardLogger()) {
            $this->attachLogger(new EbatNsBulkDataExchange_Logger(true, 'stdout', true, true));
        }
    ...
    public function curlRequestType($requestType, $apiMethod = null, $xmlns = 'http://www.ebay.com/marketplace/services')
    {
        ...
        $parser = (new EbatNsBulkDataExchange_ResponseParser($dataConverter, $callbacks, $xmlns))->withDefaultTag($responseTag);
     ...
     * @return EbatNsBulkDataExchange_Response
     */
     ...
     * @param string $method
     * @param object|array $request
     * @return EbatNsBulkDataExchange_DecodedResponse[]
     */
    public function callEach($method, $request)
    {
        /** @var EbatNsBulkDataExchange_Curler|null */
        $curler = null;
