 ...
 */
class EbatNsBulkDataExchange_ComplexType extends EbatNsBulkDataExchange_SimpleType
{
    const NAME = 'EbatNsBulkDataExchange_ComplexType';
    ...
    public function toXml($element = null, $encoder = null, $attributes = [])
    {
        ...
        foreach (self::listElements() as $tag => $info) {
            ...
            if (is_array($value)) {
                foreach ($value as $item) {
                    if ($item instanceof EbatNsBulkDataExchange_SimpleType) {
                        $elementMarkup .= $item->toXml($tag, $encoder ? $encoder : true);
                    } else { // literal
                        $elementMarkup .= self::tagify($tag, $item, [], $encoder, $type);
                    }
                }
            } elseif ($value instanceof EbatNsBulkDataExchange_SimpleType) {
        ...
        foreach ($this->_undeclaredProperties as $tag => $properties) {
            ...
            foreach ($properties as $value) {
                if ($value instanceof EbatNsBulkDataExchange_SimpleType) {
